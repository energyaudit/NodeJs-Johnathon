  ...
  fs.readFile(`./txt/${data1}.txt`, 'utf-8', (err, data2) => {
    ...
    fs.readFile('./txt/append.txt', 'utf-8', (err, data3) => {
      ...
      fs.writeFile('./txt/final.txt', `${data2}\n${data3}`, 'utf-8', err => {
        ...
        console.log(
          'this call back no data para:    Your file has been written 😁'
        );
      });
    });
  });
...
// const server = http.createServer((req, res) => {
//     const { query, pathname } = url.parse(req.url, true); //ES6 destruction to create object
//        console.log("the routing url is:   ",req.url);
//     //overview page

//     if (pathname === '/' || pathname === '/overview') {
//       res.writeHead(200, { 'Content-type': 'text/html' });
//       res.end(tempOverview);// showing {%PRODUCT_CARDS%} only so far
//     } else if (pathname === '/product') {
//       console.log(query);
//       res.writeHead(200, { 'Content-type': 'text/html' });
//       const product = dataObj[query.id]; //dataObj is array,query is object,so query.id
...
//             'Content-type': 'text/html'
//         });
////use map to loop to create ,////map  take call back as para,replace tempCard placeholder  with loop current object/element
...
// const data = fs.readFileSync(`${__dirname}/dev-data/data.json`, 'utf-8');
// //console.log(data);
// const dataObj = JSON.parse(data);//this array have 5 elements that have 5 products
// const server = http.createServer((req, res) => {
//     console.log(req.url);
...
//     if (pathName === '/' || pathName === '/overview') {
//         res.writeHead(200, { 'Content-type': 'text/html' });
//         const cardsHtml = dataObj.map(el => replaceTemplate(tempCard, el));//array of 5 iterate
